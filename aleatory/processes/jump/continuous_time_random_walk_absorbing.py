...
class AbsorbingContinuousTimeRandomWalk(BaseProcess):
    r"""

    Absorbing Continuous-Time Random Walk
    =====================================

    Notes
    -----

    An absorbing continuous-time random walk is a nearest-neighbour jump process on
    an integer state space with one-sided or two-sided absorbing boundaries.

    In this implementation, the process :math:`\{X_t : t \geq 0\}` takes values in a
    subset of :math:`\mathbb{Z}` determined by the specified absorbing boundary
    conditions.
    ...
    At an absorbing boundary, the process becomes trapped. Equivalently, once the
    process reaches an absorbing boundary, both local jump rates are set to zero and
    the state remains fixed for all subsequent times.

    So the process evolves freely in the interior until it reaches an absorbing
    boundary, after which it stays there permanently. When :math:`\lambda = \mu`,
    the walk is symmetric in the interior. When :math:`\lambda \neq \mu`, the walk
    is biased, but absorption still occurs once a boundary is hit.
    ...
    def __init__(
        ...
        :param int or None lower: lower absorbing boundary
        :param int or None upper: upper absorbing boundary
        ...
        self.name = (
            rf"Absorbing Continuous--Time Random Walk "
    ...
    def __str__(self):
        return (
            f"Absorbing continuous-time random walk with "
    ...
    def __repr__(self):
        return (
            f"AbsorbingContinuousTimeRandomWalk("
            f"rate_up={self.rate_up}, rate_down={self.rate_down}, "
            f"initial={self.initial}, lower={self.lower}, upper={self.upper})"
        )
    ...
    def _boundary_description(self):
        if self.lower is not None and self.upper is not None:
            return rf"Absorbing boundaries: $(k_{{\mathrm{{lower}}}}^*={self.lower},\ k_{{\mathrm{{upper}}}}^*={self.upper})$"
        if self.lower is not None:
            return rf"Absorbing lower boundary: $(k_{{\mathrm{{lower}}}}^*={self.lower})$"
        return rf"Absorbing upper boundary: $(k_{{\mathrm{{upper}}}}^*={self.upper})$"
